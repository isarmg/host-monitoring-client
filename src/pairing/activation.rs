...
pub(super) fn activation_endpoint(pairing_endpoint: &str) -> anyhow::Result<url::Url> {
    crate::config::validate_pairing_endpoint(pairing_endpoint)
        .context("stored pairing endpoint is unsafe")?;
    let mut endpoint =
        url::Url::parse(pairing_endpoint).context("stored pairing endpoint is not a valid URL")?;
...
pub(super) fn validate_activation_url_request(
    ...
    let url =
        url::Url::parse(activation_url).context("stored pairing activation URL is invalid")?;
    let pairing =
        url::Url::parse(pairing_endpoint).context("stored pairing endpoint is invalid")?;
...
pub(super) fn resolve_activation_url(
    ...
    let base = url::Url::parse(pairing_endpoint).context("invalid pairing endpoint URL")?;
    let url = match url::Url::parse(activation_url) {
        Ok(url) => url,
        Err(_) => base
            .join(activation_url)
    ...
    if url.scheme() != "https" {
        bail!("Host Monitoring returned an activation URL that does not use HTTPS");
    }
    Ok(url.to_string())
}
