...
适用于 0.9.14，配置与持久身份格式仍为 0.9.4。每个平台下载对应的单个原生 Release 安装包，并对照同页 SHA256SUMS 校验。安装器检查平台、架构、权限、旧版本状态并注册服务，随后使用 `setup` 完成配对、服务策略和连接验证。
...
Get-FileHash .\host-monitor-0.9.14-x64.msi -Algorithm SHA256
msiexec.exe /i .\host-monitor-0.9.14-x64.msi /norestart
...
```powershell
msiexec.exe /i "$PWD\host-monitor-0.9.14-x64.msi" REINSTALL=ALL REINSTALLMODE=amus /l*v "$env:TEMP\host-monitor-repair.log"
```

原生维护失败另写入 `C:\ProgramData\host-monitor.maintenance-diagnostic-0.9.14.txt`（管理员读取）。退出码 3010 表示需要重启完成文件替换。修复不会接管指向其他程序的同名服务，也不会追踪重解析点或删除未知数据。
...
```sh
sudo apt install ./host-monitor_0.9.14_amd64.deb
sudo host-monitor setup
```

RPM 系统使用 `sudo dnf install ./host-monitor-0.9.14.x86_64.rpm`；同版损坏重装可用 `sudo rpm -Uvh --replacepkgs ./host-monitor-0.9.14.x86_64.rpm`。DEB 同版重装使用 `sudo apt install --reinstall ./host-monitor_0.9.14_amd64.deb`。不要添加忽略依赖的参数。
...
```sh
sudo installer -pkg ./host-monitor-0.9.14-macos-arm64-unsigned.pkg -target /
