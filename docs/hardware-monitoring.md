...
默认每五分钟采集一次，允许 60–86400 秒。自动发现仅检查系统安装位置，包括 Linux `/usr/sbin/smartctl`、`/usr/bin/smartctl`，macOS `/opt/homebrew/sbin/smartctl`，Windows `C:\Program Files\smartmontools\bin\smartctl.exe`。自定义 `executable` 必须是绝对路径。程序不安装 smartmontools，也不自动提升服务权限。设备访问权限不足会报告 `permission_denied`；工具缺失报告 `driver_missing`。
